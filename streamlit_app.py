...
KNOWN_STOCK_SYMBOLS = set(ASSET_MAPPING[key] for key in ASSET_MAPPING if key in ["APPLE", "TESLA", "MICROSOFT", "AMAZON", "GOOGLE", "NVIDIA", "FACEBOOK", "MICROSTRATEGY", "MSTR", "WALMART", "NASDAQ", "NDX"])
...
def resolve_asset_symbol(input_text, asset_type, quote_currency="USD"):
    ...
    if asset_type == "Crypto":
        ...
        # Stock/Index symbols are the ticker itself (e.g., TSLA, NDX)
...
# --- API HELPERS (UNCHANGED, return None on failure) ---
...
# === UNIVERSAL PRICE FETCHER (No Fallbacks) ===
...
# === HISTORICAL DATA & INDICATOR PLACEHOLDERS (UNCHANGED) ===
...
def supertrend_status(df): return "Bullish"
def bollinger_status(df): return "Within Bands — Normal"
...
def get_kde_rsi_explanation(): return "KDE RSI uses probability density to identify overbought/oversold conditions more accurately than traditional RSI."
...
def get_bollinger_explanation(status):
    ...
    elif "Upper" in status: return "Price is touching upper band - potential overbought condition or strong trend."
    else: return "Price is touching lower band - potential oversold condition or weak trend."
...
def get_psar_explanation(status):
    if "Bullish" in status: return "SAR dots below price confirm the <strong>uptrend</strong> and provide trailing stop levels for long positions."
...
def combined_bias(kde_val, st_text, ema_status):
    ...
    if kde_val > 60 and is_bullish_trend: return "Strong Bullish"
    if kde_val < 40 and is_bearish_trend: return "Strong Bearish"
    if 40 <= kde_val < 60: return "Neutral (Consolidation/Wait for Entry Trigger)"
...
def analyze(symbol, price_raw, price_change_24h, vs_currency, asset_type):
    ...
    if price_raw is None:
        return generate_error_message(
            ...
            details="Please ensure the ticker symbol is correct for the selected asset type. If the symbol is correct, the underlying data providers may be temporarily unavailable or your API key is invalid/rate-limited."
        )
...
ndx_base_symbol, ndx_symbol = resolve_asset_symbol("NDX", "Stock/Index", "USD")
