...
t    current_price = price_raw if price_raw is not None and price_raw > 0 else df_15m["close"].iloc[-1] 
    
...
data    psar_status = parabolic_sar_status(symbol, kde_val) 
    
...
        entry = current_price 
        target = current_price - (2.5 * atr_val)
A        stop = current_price + (1.0 * atr_val)
    
    motivation = {
...
  _   if ratio < 20: status = "Flat / Very Low Volatility"
